...
class ErrorBoundary extends React.Component {
    ...
    componentDidCatch(error, errorInfo) {
        console.error("WorkerDashboard Error:", error, errorInfo);
    }
...
function WorkerDashboardContent() {
    ...
    useEffect(() => {
        ...
        if (data.requirePasswordChange || data.adminData?.require_password_change || data.admin_data?.require_password_change) {
            setMustChangePassword(true);
        }

        const loadWorker = async () => {
            try {
                // Sync with store to get latest status/documents using getById (more efficient and RLS-friendly)
                const currentWorker = await workerStore.getById(data.id);
                ...
                // If we absolutely can't load, use session data
                setWorker(data);
            }
        };
        loadWorker();
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50">
                <div className="flex flex-col items-center gap-4">
                    <Loader2 className="w-8 h-8 animate-spin text-scafoteam-navy" />
                    <p className="text-sm font-medium text-gray-500">Ielādē datus...</p>
                </div>
            </div>
        );
    }
    ...
    const handleMandatoryPasswordChange = async (e) => {
        ...
        if (!/(?=.*[0-9])(?=.*[a-zA-Z])/.test(passwords.new)) {
            alert('Parolei jāsatur gan burti, gan cipari!');
            return;
        }
        ...
        try {
            const res = await workerStore.updateAuthPassword(passwords.new);
            if (res.success) {
                setMustChangePassword(false);
                // Update local storage/session to reflect the change
                const updatedWorker = { ...worker, requirePasswordChange: false };
                setWorker(updatedWorker);
                sessionStorage.setItem('workerData', JSON.stringify(updatedWorker));

                alert('Parole veiksmīgi nomainīta! Tagad vari turpināt darbu.');
            } else {
    ...
    const handleSaveSignature = async (signatureDataUrl) => {
        ...
        try {
            ...
            toast.error('Kļūda parakstīšanā: ' + (error.message || 'Nezināma kļūda'));
        } finally {
            setIsSigning(false);
        }
    };
    ...
    return (
        <Layout className="max-w-6xl pb-20">
            ...
            <Modal
                ...
                title="Svarīga drošības pārbaude"
                className="max-w-md"
                showCloseButton={false}
            >
                <div className="space-y-6">
                    <div className="p-4 bg-blue-50 rounded-2xl border border-blue-100">
                        <p className="text-sm font-bold text-scafoteam-navy leading-relaxed">
                            Lai nodrošinātu Tavus datus, Tev ir jāiestata jauna, droša parole.
                        </p>
                        <ul className="mt-3 space-y-1 text-xs text-blue-800 font-medium">
                            <li className="flex items-center gap-2">• Vismaz 8 rakstzīmes</li>
                            <li className="flex items-center gap-2">• Gan burti, gan cipari</li>
                        </ul>
                    </div>
                    ...
                    <form onSubmit={handleMandatoryPasswordChange} className="space-y-4">
                        <div className="space-y-2">
                            ...
                            <Input
                                ...
                                placeholder="Pievienot paroli..."
                            />
                        </div>
                        <div className="space-y-2">
                            <label className="text-xs font-black text-gray-400 uppercase tracking-widest">Apstiprināt paroli</label>
                            <Input
                                type="password"
                                required
                                value={passwords.confirm}
                        ...
                        <Button
                            ...
                            className="w-full h-12 bg-scafoteam-navy hover:bg-scafoteam-navy/90 text-white font-black uppercase tracking-widest rounded-xl mt-4"
                        >
                            {isSavingSettings ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : <Lock className="w-4 h-4 mr-2" />}
